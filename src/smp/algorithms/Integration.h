...
namespace pmp
{
	...
	public:
		Integration(SurfaceMesh& mesh, const cv::Mat& normals, const cv::Mat& mask = cv::Mat(), Camera camera = Camera()) : mesh_(mesh_), geo_(mesh_, normals, mask, camera), projection_(camera)
		{
