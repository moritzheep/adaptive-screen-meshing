...
#include "smp/algorithms/ScreenRemeshing.h"
...
public:
    PhotometricRemeshing(const cv::Mat& normals, const cv::Mat& mask, Projection projection = Projection()) : normals_(normals), mask_(mask), projection_(projection)
    {
    ...
    {
        // Create Quadmesh
        std::function<bool(int, int)> indicator = [&](int v, int u){ return (u != 0) && (v != 0) && (u != width_ - 1) && (v != height_ - 1) && mask_.at<uchar>(v, u) > 127; };
